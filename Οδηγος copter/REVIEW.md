...
> **Κατάσταση: όλα τα ευρήματα διορθώθηκαν** στα αρχεία αυτού του φακέλου. Το κείμενο
> παρακάτω παραμένει ως τεκμηρίωση του ελέγχου — οι αριθμοί γραμμών αναφέρονται στην
> **προ των διορθώσεων** έκδοση.

**Αντικείμενο ελέγχου:** `copter-tuning-guide/` στο branch `claude/ardupilot-tuning-course-s28nkl`
(13 αρχεία, 4.173 γραμμές, 46 διαγράμματα mermaid). Το ίδιο περιεχόμενο βρίσκεται και στον
φάκελο «Οδηγος copter/» του `master`· οι διορθώσεις εφαρμόστηκαν εκεί.
...
Επαληθεύτηκε και η παλιά τιμή: στο tag `Copter-4.6.0` του upstream ArduPilot, το ίδιο
`#define` είναι `0.15f // Medium` για Copter/Sub (και `0.2f // Soft` για Plane). Δηλαδή στην
4.7 μειώθηκαν και τα δύο κατά ένα σκαλί.
...
Όλα τα παραπάνω εφαρμόστηκαν:

| Εύρημα | Διόρθωση |
|---|---|
| Ε1 | Το default διορθώθηκε σε 0.10 στο κεφ. 7· ο πίνακας αίσθησης ξαναγράφτηκε με τις ονομασίες των `@Values` του κώδικα· προστέθηκε νέα ενότητα **Α.9 «Ίδιο όνομα, διαφορετικό default»** στο Παράρτημα |
| Ε2, Ε3 | `PSC_JERK_D` → `PSC_D_JERK` και `PSC_JERK_NE` → `PSC_NE_JERK` σε 9 σημεία, συμπεριλαμβανομένων των mermaid |
| Ε4 | Οι τρεις γραμμές έγιναν «ίδια ⚠» και προστέθηκε προειδοποίηση με την πραγματική συνέπεια· η εισαγωγή του Παραρτήματος δεν υπόσχεται πια ανεπιφύλακτα αυτόματη μετατροπή |
| Μ1 | Προστέθηκε το × 0.01 και παράδειγμα `ANGLE_MAX = 4500` → `ATC_ANGLE_MAX = 45` |
| Μ2 | Συμπληρώθηκαν και τα 7 defaults του πίνακα FFT, με έμφαση στο `FFT_HMNC_FIT = 10` |
| Μ3 | Προστέθηκε ο ακριβής τύπος του `sqrt_controller()` ως σημείωση κάτω από το διάγραμμα |
| Μ4 | Προστέθηκε η προειδοποίηση «μη τη ρυθμίζεις χειροκίνητα» |
| Μ5 | Παραπομπή από το κεφ. 9 προς το §8.13 για τα κάθετα kinematics |
| Μ6 | Εξήγηση στο README γιατί η αρίθμηση ξεκινά από το 02 |

Μετά τις αλλαγές ξανατρέξανε και οι τρεις έλεγχοι: κανένα άγνωστο όνομα παραμέτρου
(διασταύρωση με τις 5.771 του `apm.pdef.json`), κανένας σπασμένος σύνδεσμος, και τα 46
διαγράμματα mermaid εξακολουθούν να κάνουν parse.
