...
Το σύνορο μεταξύ των δύο περιοχών (`linear_dist` στον κώδικα) είναι `accel_lim / P²`.
Ψηλότερο P σημαίνει μικρότερη γραμμική περιοχή.

> **Ο ακριβής τύπος:** το διάγραμμα δείχνει `√(2 × accel_lim × σφάλμα)` για ευκολία. Ο
> κώδικας (`sqrt_controller()` στο `AP_Math/control.cpp`) υπολογίζει
> `√(2 × accel_lim × (σφάλμα − linear_dist/2))`. Ο όρος `− linear_dist/2` υπάρχει ώστε οι
> δύο κλάδοι να ενώνονται ομαλά στο σύνορο· δεν αλλάζει τίποτα στη λογική του tuning.
...
| `ATC_INPUT_TC` | 0.10 s | **Χρονική σταθερά εισόδου** — πόσο "απαλά" μεταφράζεται η κίνηση του stick |
| `ATC_RATE_FF_ENAB` | 1 | Body-frame rate feedforward από τον angle controller |

### `ATC_INPUT_TC` — το αίσθημα του drone

Οι ονομασίες είναι αυτές που δίνει ο ίδιος ο κώδικας στα `@Values` της παραμέτρου:

| Τιμή | Αίσθημα |
|---|---|
| 0.05 | Very Crisp — πολύ άμεσο, "sport" |
| **0.10** | **Crisp — το default του Copter** |
| 0.15 | Medium (είναι το default του Plane, όχι του Copter) |
| 0.20 | Soft — καλό για κάμερα ή μεγάλα drones |
| 0.50 | Very Soft |

> **Προσοχή αν διαβάζεις παλιότερο υλικό:** μέχρι και την 4.6 το default του Copter ήταν
> **0.15**. Στην 4.7 μειώθηκε σε **0.10** (PR 32643). Το όνομα της παραμέτρου δεν άλλαξε,
> οπότε η αλλαγή περνάει εύκολα απαρατήρητη — δες [Παράρτημα Α](A-Αντιστοίχιση-Παραμέτρων.md).
